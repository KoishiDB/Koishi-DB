...
class UserComparator: public Comparator {
  ...
  TEST(Table_test, BasicTest) {
    ...
    EXPECT_EQ(table_builder.NumEntries(), 100);
    ...
    for (int i = 1; i <= 100; i++) {
      std::string key = "temp_test_key" + std::to_string(i);
      std::string value = "temp_test_value" + std::to_string(i);
      EXPECT_EQ(sstable_iter->Key(), key);
      EXPECT_EQ(sstable_iter->Value(), value);
