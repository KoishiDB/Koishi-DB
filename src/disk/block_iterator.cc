...
#include "util/util.h"
namespace koishidb {
BlockIterator::BlockIterator(Block *block, const Option *opt)
    : data_(block->data), size_(block->size), cmp_(opt->cmp) {
  SeekToFirst();
}
...
Slice BlockIterator::Key() const { return key_; }

Slice BlockIterator::Value() const { return value_; }

bool BlockIterator::Valid() const { return offset_ < size_; }
