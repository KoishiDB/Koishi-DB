...
TableBuilder::TableBuilder(const Option* opt, WritableFile* writableFile)
    : rep_(new Rep(opt, writableFile)) {}
...
void TableBuilder::Add(const Slice& key, const Slice& value) {
  ...
  assert(r->last_key.empty() ||
         r->opt->cmp->Compare(key, r->last_key.c_str()) == 1);
  ...
  r->last_key.assign(
      key.data(),
      key.size());  // use std::string.assign(), can hold string or c_str
  r->num_entries++;
...
void TableBuilder::WriteBlock(BlockBuilder* block_builder,
...
Status TableBuilder::Finish() {
  ...
  // currently we have write the all the data blocks and build up a new index
  // block what we need to do is to write them to the table file. 1.Write Filter
  // Blocks-> 2.Write MetaIndex Block 3.Write IndexBlock 4.Write Footer
  ...
  // filter_block_handle, index_block_handle, paddings(40 -
  // sizeof(filter_block_handle) - sizeof(index_block_handler))
  Footer footer;
