...
Slice SSTableIterator::Value() const { return data_iter_->Value(); }

Slice SSTableIterator::Key() const { return data_iter_->Key(); }

bool SSTableIterator::Valid() const { return data_iter_->Valid(); }
bool SSTableIterator::Seek(const Slice& target) {
  index_iter_->Seek(target);
  Slice block_handle = index_iter_->Value();
  if (std::string(block_handle.data(), block_handle.size()) !=
      data_block_handle_) {
    UpdateDataIterator();
  }
