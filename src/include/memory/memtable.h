...
// For memtable key, iff the user_key is the same, then the larger the sequence
// is, that smaller the memtablekey is.
class MemtableKeyComparator : public Comparator {
 public:
  ...
  std::string Name() const { return "MemtableKeyComparator"; }
};
...
class Memtable {
 ...
 private:
  ...
  std::shared_mutex rwlock_;  // reader and writer lock, when is_mutable_ ==
                              // true, should be used
